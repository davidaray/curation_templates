...
#Paths
TAXON=<NAME>
GENOME=/lustre/scratch/daray/bat1k_TE_analyses/assemblies/${TAXON}.fa.gz		
WORKDIR=/lustre/scratch/daray/bat1k_TE_analyses/te_curations/${TAXON}
CONSENSUSFILE=$WORKDIR/rmodeler_dir/${TAXON}-families.fa
...
##Get RepeatModeler consensus file info and preprocess file
echo "Original queries file is "$CONSENSUSFILE
echo "Modifying to remove consensus sequences <100 bp"
echo "New consensus file is "${TAXON}-families.mod.fa 
cd $WORKDIR/rmodeler_dir/
perl $GITPATH/removesmalls.pl 100 $CONSENSUSFILE | cut -d"#" -f1 >${TAXON}-families.mod.fa
sed -i "s/rnd-/${TAXON}-rnd-/g" ${TAXON}-families.mod.fa
CONSENSUSFILE=$WORKDIR/rmodeler_dir/${TAXON}-families.mod.fa
