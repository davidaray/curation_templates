    ...
    with open(mapping_file, 'r') as file:
        ...
        for line in file:
            ...
            if not line:  # Skip empty lines
                continue
            columns = line.split('\t')
            if len(columns) < 6:  # Ensure the line has all required columns
                print(f"Warning: Skipping malformed line in mapping file: {line}")
                continue
            tax_family = columns[0]       # Taxonomic_Family
            species_id = columns[2]       # Species_ID
...
def extract_class_family_data(file_path, species_family_map):
    ...
    te_family_data = {te_class: defaultdict(int) for te_class in te_classes}
    
    species = os.path.basename(file_path).split('.fa.out.gz')[0]
    ...
    with gzip.open(file_path, 'rt') as file:
        for line in file:
            ...
            if len(columns) > 10:  # Ensure there are enough columns
                te_class_family = columns[10]  # Class/Family is in column 11
                te_class = te_class_family.split('/')[0]
                te_family = te_class_family.split('/')[1] if '/' in te_class_family else "Unknown"

                if te_class in te_classes:  # Only count known classes
                    te_class_data[te_class] += 1
                    te_family_data[te_class][te_family] += 1
    
    return species, tax_family, te_class_data, te_family_data

# Function to build matrices for classes and families using multiprocessing
def build_matrices(file_list, species_family_map, num_proc):
    ...
    family_matrices = {te_class: pd.DataFrame() for te_class in te_classes}
    ...
    for species, tax_family, te_class_data, te_family_data in results:
        ...
        for te_class, family_counts in te_family_data.items():
            family_matrices[te_class] = pd.concat([family_matrices[te_class], pd.DataFrame(family_counts, index=[species])], axis=0).fillna(0)
...
def save_matrices(class_matrix, family_matrices, output_dir):
    ...
    class_matrix.to_csv(os.path.join(output_dir, 'class_PCA_count_matrix.tsv'), sep='\t')
    
    for te_class, df in family_matrices.items():
        df.to_csv(os.path.join(output_dir, f'{te_class}_family_PCA_count_matrix.tsv'), sep='\t')

# Function to create a human-readable title from output_prefix
def make_plot_title(output_prefix, biplot=False):
    basename = os.path.basename(output_prefix)
    if basename == "class_matrix":
        title = "TE Class Insertion Counts"
    elif basename.endswith("_family_matrix"):
        class_name = basename.replace("_family_matrix", "")
        title = f"{class_name} Insertion Counts"
    else:
        title = basename
    return f'PCA Biplot for {title}' if biplot else f'PCA Plot for {title}'

# Function to run PCA and generate plots along with loadings
def run_pca_and_plot(matrix, output_prefix, species_family_map):
    ...
    custom_markers = {
        "Pteropodidae": "o", "Rhinopomatidae": "s", "Megadermatidae": "D", "Craseonycteridae": "^",
        "Rhinonycteridae": "v", "Hipposideridae": "<", "Rhinolophidae": ">", "Emballonuridae": "p",
        "Nycteridae": "H", "Mystacinidae": "*", "Thyropteridae": "X", "Furipteridae": "8",
        "Noctilionidae": "P", "Mormoopidae": "d", "Phyllostomidae": "h", "Myzopodidae": "o",
        "Natalidae": "s", "Molossidae": "d", "Miniopteridae": "p", "Vespertilionidae": "H",
        "Cistugidae": "X", "Unknown": "o"
    }
    ...
    # Save PCA results
    pca_df.to_csv(f'{output_prefix}_PCA_count_scores.tsv', sep='\t')
    ...
    plt.title(make_plot_title(output_prefix, biplot=False))  # 🔹 Updated title
    ...
    legend = plt.legend(
        ...
        title='Taxonomic Family', frameon=True, handlelength=4.0
    )
    marker_size = 100
    ...
    plt.savefig(f'{output_prefix}_PCA_count_plot.png', bbox_inches='tight')
    plt.close()
    ...
    plt.title(make_plot_title(output_prefix, biplot=True))  # 🔹 Updated title
    plt.xlabel(f'PC1 - {pca.explained_variance_ratio_[0]*100:.2f}%')
    plt.ylabel(f'PC2 - {pca.explained_variance_ratio_[1]*100:.2f}%')
    ...
    plt.tight_layout()
    plt.savefig(f'{output_prefix}_PCA_count_biplot.png')
    plt.close()

    # Save PCA results to file
    pca_df.drop(columns='Taxonomic_Family').to_csv(f'{output_prefix}_PCA_count_result.tsv', sep='\t')
    pca_df[['PC1', 'PC2', 'Taxonomic_Family']].to_csv(f'{output_prefix}_count_coordinates.tsv', sep='\t')

    loadings = pca.components_
    loadings_df = pd.DataFrame(loadings.T, index=matrix.columns, columns=['PC1', 'PC2'])
    loadings_df.to_csv(f'{output_prefix}_PCA_count_loadings.tsv', sep='\t')

    print(f'PCA Loadings for {output_prefix} saved to {output_prefix}_PCA_count_loadings.tsv')
...
def main():
    ...
    class_matrix_path = os.path.join(args.output_dir, 'class_PCA_count_matrix.tsv')
    family_matrix_paths = {te_class: os.path.join(args.output_dir, f'{te_class}_family_PCA_count_matrix.tsv') for te_class in te_classes}
    ...
    if not args.skip_calculations and not matrices_exist:
        ...
        family_matrices = {te_class: pd.read_csv(family_matrix_paths[te_class], sep='\t', index_col=0) for te_class in te_classes}
