...
'''
# Function 4b: Run RepeatModeler on target genome using conda installation
def RMODEL(GENOMEPREFIX, PROCESSORS, WORKPATH):
    # Change directory to the assemblies directory
    assemblies_dir = os.path.join(WORKPATH, 'assemblies_dir')
    os.chdir(assemblies_dir)
    
    # Decompress the .fa.gz file
    with gzip.open(GENOMEPREFIX + '.fa.gz', 'rb') as GENOMEIN:  
        with open(GENOMEPREFIX + '.fa', 'wb') as GENOMEOUT:
            shutil.copyfileobj(GENOMEIN, GENOMEOUT)
    
    # Change directory to the rmodeler directory
    rmodeler_dir = os.path.join(WORKPATH, 'rmodeler_dir')
    os.chdir(rmodeler_dir)

    # Activate conda environment and run RepeatModeler commands
    subprocess.run(
        'source ~/conda/etc/profile.d/conda.sh && conda activate repeatmodeler && '
        'BuildDatabase -name {} {} && '
        'RepeatModeler -database {} -threads {} > {}.RMrun.out'.format(
            GENOMEPREFIX, os.path.join(assemblies_dir, GENOMEPREFIX + '.fa'), 
            GENOMEPREFIX, PROCESSORS, GENOMEPREFIX
        ), 
...
# Run RepeatModeler
	RMODEL(GENOMEPREFIX, PROCESSORS, WORKPATH)
