...
def read_repeatmasker_file(filepath, progress_dir=None):
    ...
    if progress_dir:
        ...
        read_progress = progress_path / "00_reading_input.txt"
        with open(read_progress, 'w') as f:
            f.write(f"Started reading: {filepath}\n")
    ...
    if progress_dir:
        with open(read_progress, 'a') as f:
    ...
    for i, line in enumerate(lines[3:], start=4):
        ...
        if progress_dir and (i - 3) % 100000 == 0:
            with open(read_progress, 'a') as f:
                f.write(f"Parsed {i - 3} lines, {len(elements)} valid elements\n")
    
    print(f"Parsed {len(elements)} valid elements")
    
    if progress_dir:
        with open(read_progress, 'a') as f:
            f.write(f"Finished reading at: {__import__('datetime').datetime.now()}\n")
            f.write(f"Total valid elements: {len(elements)}\n")
            if parse_errors > 0:
...
def main():
    ...
    header, elements = read_repeatmasker_file(args.input, args.progress_dir)
