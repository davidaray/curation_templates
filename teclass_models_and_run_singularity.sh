...
#conda activate TEclass2-test
conda activate TEclass2

WORKDIR=/lustre/scratch/daray/bat1k_TE_analyses/teclass2_singularity
...
singularity run -B $WORKDIR:$WORKDIR,$BIN:$BIN teclass2_container_spython_v4.sif \
	--database \
	-c $WORKDIR/config.yml \
	>$WORKDIR/database_output.txt
: << 'EOF'
