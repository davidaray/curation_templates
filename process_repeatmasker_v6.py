...
class RepeatElement:
    ...
    def to_bed(self) -> str:
        ...
        return f"{self.scaffold}\t{self.start-1}\t{self.end}\t{self.repeat_name}\t{self.score}\t{strand_bed}\t{self.repeat_class}"
...
def resolve_group(group: List[RepeatElement], resolution: str) -> List[RepeatElement]:
    ...
    for elem in group[1:]:
        new_active = []
        elem_processed = False
        
        for curr in active:
            if curr.overlaps(elem):
                winner, loser = resolve_overlap_pair(curr, elem, resolution)
                new_active.append(winner)
                if loser is not None:
                    if loser == elem:
                        elem_processed = True
                    else:
                        new_active.append(loser)
                else:
                    if winner == elem:
                        elem_processed = True
            else:
        ...
        if not elem_processed:
            new_active.append(elem)
