...
WORKDIR=/lustre/scratch/daray/bat1k_TE_analyses/teclass2_cuda
...
# Alter config.yml to include elements in database with at least 400 entries. 500 is recommended but I want to ensure that Rhin SINEs are included. There are 420 of them in the library. 
: << 'EOF'
...
te_keywords=$(awk '$NF > 400 {print $1}' database_output.txt | grep -v 'total_len\|Dataset' | tr '\n' ',' | sed 's/,$//')
echo "te_keywords = ${te_keywords}"

# Remove all entries not used for training.
mv ${WORKDIR}/data/bats_v1.fa ${WORKDIR}/data/bats_v1_full.fa
python teclass2_database_prep.py \
	-i ${WORKDIR}/data/bats_v1_full.fa \
	-l ${te_keywords} \
	-o ${WORKDIR}/data/bats_v1.fa

# Preparing the te_keywords line for config.yml
te_keywords="te_keywords: [\"$(echo $te_keywords | sed 's/,/","/g')\"]"
echo "te_keywords for insertioninto config file = ${te_keywords}"
...
grep ">" ${WORKDIR}/data/bats_v1.fa | sed "s/>//g" | sort | uniq >training_labels.txt
